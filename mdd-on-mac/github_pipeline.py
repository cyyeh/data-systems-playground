...
def load_vulcansql_events() -> None:
    """Loads vulcansql events. Shows incremental loading. Forces anonymous access token"""
    pipeline = dlt.pipeline(
        "pipeline_vulcansql_events", destination='duckdb', dataset_name="vulcansql_events"
    )
    data = github_repo_events("canner", "vulcan-sql", access_token="")
    print(pipeline.run(data))
